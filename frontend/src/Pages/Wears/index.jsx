...
export default function Shoe() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const filteredwears = data?.data?.filter((e) => {
          return e?.attributes?.Category == 'wears';
        });
        setwears(filteredwears);
        if (filteredwears.length > 0) {
          const prices = filteredwears.map((e) => +e.attributes.Price);
  ...
  useEffect(() => {
    if (search) {
      const filteredItems = wears?.filter((e) => {
